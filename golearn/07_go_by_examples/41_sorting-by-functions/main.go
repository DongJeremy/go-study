...
// 有时候，我们可能想根据自然顺序以外的方式来对集合进行排序。 例如，假设我们要按字符串的长度
// 而不是按字母顺序对它们进行排序。 这儿有一个在 Go 中自定义排序的示例。

// 为了在 Go 中使用自定义函数进行排序，我们需要一个对应的类型。 我们在这里创建了一个 byLength
// 类型，它只是内建类型 []string 的别名。
...
// 然后对这个自定义类型的切片调用 sort.Sort 方法， 我们就可以通过任意函数对 Go 切片进行排序了。
