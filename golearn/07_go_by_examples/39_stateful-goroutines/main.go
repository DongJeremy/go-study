...
func main() {
	...
	// 这就是拥有 state 的那个协程， 和前面例子中的 map 一样，不过这里的 state
	// 是被这个状态协程私有的。 这个协程不断地在 reads 和 writes 通道上进行选择，
	// 并在请求到达时做出响应。 首先，执行请求的操作；然后，执行响应，在响应通道 resp
	// 上发送一个值，表明请求成功（reads 的值则为 state 对应的值）。
	go func() {
...
// 运行这个程序显示这个基于协程的状态管理的例子 达到了每秒大约 800,000 次操作。
