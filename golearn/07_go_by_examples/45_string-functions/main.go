...
func main() {
	...
	// 这意味着我们需要在调用函数时，将字符串作为第一个参数进行传递。 你可以在 strings
	// 包文档中找到更多的函数。
	...
	// 虽然不是 strings 的函数，但仍然值得一提的是， 获取字符串长度（以字节为单位）
	// 以及通过索引获取一个字节的机制。
	p("Len: ", len("hello"))
	p("Char:", "hello"[1])
}
...
// 如果您可能使用多字节的字符，则需要使用可识别编码的操作。 详情请参考 strings, bytes,
// runes and characters in Go。
