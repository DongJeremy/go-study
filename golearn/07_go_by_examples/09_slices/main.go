...
func main() {
	// 与数组不同，slice 的类型仅由它所包含的元素的类型决定（与元素个数无关）。 要创建一个长度
	// 不为 0 的空 slice，需要使用内建函数 make。 这里我们创建了一个长度为 3 的 string
	// 类型的 slice（初始值为零值）。
	s := make([]string, 3)
	fmt.Println("emp:", s)
	...
	// 该函数会返回一个包含了一个或者多个新值的 slice。 注意由于 append 可能返回一个新的
	// slice，我们需要接收其返回值。
	s = append(s, "d")
	s = append(s, "e", "f")
	fmt.Println("apd:", s)
	...
	// slice 支持通过 slice[low:high] 语法进行“切片”操作。 例如，右边的操作可以得到
	// 一个包含元素 s[2]、s[3] 和 s[4] 的 slice。
	l := s[2:5]
	fmt.Println("sl1:", l)
